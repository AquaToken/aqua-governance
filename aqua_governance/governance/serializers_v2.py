...
class SubmitSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        if self.instance.proposal_type in (Proposal.ASSET_WHITELIST, Proposal.ASSET_REVOCATION):
            duration = attrs['new_end_at'] - attrs['new_start_at']
            min_duration = timedelta(days=settings.ASSET_PROPOSAL_MIN_VOTING_DAYS)
            if duration < min_duration:
                raise serializers.ValidationError(
                    {'new_end_at': ['Asset proposals require a minimum voting period of 10 days.']}
                )
        return attrs
